...
from llm.llm_interface import build_prompt
...
def query_lm_studio(client, prompt_text, model_name="local-model"):
    """Sends a query to the LM Studio server and measures performance."""
    try:
        start_time = time.perf_counter()
        
        response = client.chat.completions.create(
            model=model_name, # This is often ignored by LM Studio, which uses the loaded model
...
def run_main_py_logic(user_query, client, model_name="local-model"):
    """Runs the logic from main.py and evaluates it."""
    ...
    # 3️⃣ Send to LLM and measure performance
    result = query_lm_studio(client, prompt, model_name)
...
def evaluate_model(model_config, client):
    ...
    for i, prompt in enumerate(sample_prompts):
        ...
        result = run_main_py_logic(prompt, client, model_config['name'])
        if result:
...
if __name__ == "__main__":
    ...
    MODEL_1_CONFIG = {
        "name": "Qwen/Qwen2-1.5B-Instruct-GGUF/qwen2-1.5b-instruct-q8_0.gguf",
        "hypothetical_cost_per_input_token": 0.20 / 1_000_000,
        "hypothetical_cost_per_output_token": 1.00 / 1_000_000
    }

    MODEL_2_CONFIG = {
        "name": "google/gemma-2b-it-gguf/gemma-2b-it-q8_0.gguf",
        "hypothetical_cost_per_input_token": 0.20 / 1_000_000,
        "hypothetical_cost_per_output_token": 1.00 / 1_000_000
    }
